...
import { compStatusChange, FlagChange } from '../../Actions';
...
const PemberianMakanan1 = ( props ) => {
  ...
  useEffect( ()=>{
    ...
    else if(klasifikasiBatuk.bsb_status === 'danger' || klasifikasiBatuk.bsb_status === 'warning'){
      cek = true;
    }
    else if(klasifikasiDiare.diare_status === 'danger' || klasifikasiDiare.diare_status === 'warning'){
      cek = true;
    }
    else if(klasifikasiDemam.demam_status === 'danger' || klasifikasiDemam.demam_status === 'warning'){
      cek = true;
    }
    else if(klasifikasiTelinga.telinga_status === 'danger' || klasifikasiTelinga.telinga_status === 'warning'){
      cek = true;
    }
    else if(klasifikasiGizi.gizi_status === 'danger' || klasifikasiGizi.gizi_status === 'warning'){
      cek = true;
    }
    else if(klasifikasiHIV.hiv_status === 'danger' || klasifikasiHIV.hiv_status === 'warning'){
      cek = true
    }
    if(cek) {
      set_rujukSegera('Ya');
  ...
  const handleSubmit = event =>{
    ...
    if((umur < 2 || ansGizi.gizi_BBmenurutPBAtauTB === '-3 SD ≥ BB/PB (TB) ≤ -2 SD (Kurus)' || klasifikasiAnemia.anemia_status === 'danger') && rujukSegera === 'Tidak'){
  ...
  return(
    <Form onSubmit={handleSubmit}>
              ...
              <Row className="justify-content-center">
                  <Card style={outlineColor} className="text-justify w-75 mt-3">
                      <CardBody>
                          ...
                          <Row className="justify-content-center">
                            <Label>Anemia: {(klasifikasiAnemia.anemia_status === 'danger') ? 'Ya' : ' Tidak'}</Label>
                          </Row>
                          <Row className="justify-content-center">
        ...
        <Row className="justify-content-between px-5 py-0">
            <Col sm="4">
                <Link to="TandaBahayaUmum1" style={{textDecoration: "none"}}><Button block className="button-orange"><FontAwesomeIcon icon={faChevronLeft}/> Pemeriksaan Keluhan Lain</Button></Link>
            </Col>
            <Col sm="4">
                <Button type="submit" block className="button-pepper-mint">Selanjutnya <FontAwesomeIcon icon={faChevronRight}/></Button>
            </Col>
        </Row>
      </div>
    </Form>
